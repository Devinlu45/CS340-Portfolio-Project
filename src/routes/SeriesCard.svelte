<script lang="ts">
	...
	import type { Movie, Platform, Series } from '$lib/database';
	import Modal from './Modal.svelte';

	export let series: Series;
	export let platformIDs: number[];
	let randomImage = `https://source.unsplash.com/300x300/?${series.Name}`;
...
<div class="flex w-64 flex-col justify-between rounded-sm bg-white p-5 shadow-md">
	<div>
		<img src={randomImage} alt={series.Name} class="h-32 w-full rounded-lg object-cover" />
		<h2 class="mt-5 text-xl font-bold">{series.Name}</h2>
	</div>
...
<Modal showModal={showEdit}>
	<h2 slot="header">{series.Name}</h2>
	<form method="POST" use:enhance>
		<div class="flex flex-col gap-2">
			<label>
				<span class="text-sm">Name</span>
				<input type="text" name="Name" class="w-full bg-gray-100 p-2" value={series.Name} />
			</label>
			<label>
				<span class="text-sm">URLs</span>
				<textarea name="URLs" class="w-full bg-gray-100 p-2" rows="5">{series.URLs}</textarea>
			</label>
			<label>
				<span class="text-sm">Episode Count</span>
				<input
					type="number"
					name="Episode_Count"
					class="w-full bg-gray-100 p-2"
					value={series.Episode_Count}
				/>
			</label>
			<label>
				<span class="text-sm">Season Count</span>
				<input
					type="number"
					name="Season_Count"
					class="w-full bg-gray-100 p-2"
					value={series.Season_Count}
				/>
			</label>
...
<Modal showModal={showDelete}>
	<h2 slot="header">Are you sure?</h2>
	<p>Do you really want to delete <b>{series.Name}</b>?</p>
	<button
		slot="button"
